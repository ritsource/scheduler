...
export const SubOptColorComp = (props) => {
  ...
  return (
    <div style={{ background: 'var(--background-color)' }}>
      ...
      {input_visible ? (
        <form style={{ maxWidth: '145px', marginTop: '-2px' }}
        ...
        >
          <div style={{ fontSize: '18px' }}>
            {new_color_hex && (
              <FaCircle style={{ color: new_color_hex, margin: '0px 10px 0px 0px' }}/>
            )}
          </div>
          <input
            style={{ padding: '4px' }}
      ...
      {submit_error && (
        <p className='calendar-sidebar-color-error_message'>{submit_error}</p>
      )}
